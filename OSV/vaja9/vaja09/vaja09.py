...
def affineRegistration(iType, rCP, iCP, iImage):
    ...
    if iType == "interpolation":
        # interpoliramo matriki XY in UV z 1 povsod.
        XY = np.ones((3, 3))
        UV = np.ones((3, 3))

        # prvi dve vrstici zapolnimo s koordinatami kontrolnih in referenčnih točk.
        XY[0], XY[1] = X, Y
        UV[0], UV[1] = U, V

        # če je determinanta matrike = 0, potem ne moremo izračunati matrike
        if np.linalg.det(XY) == 0:
            raise ValueError("Matrika ni invertibilna, izberi boljše točke.")
        
        else:
        # izračunamo matriko T preslikave med XV in UV
    ...
    # inicializiranje tock UV za nadaljno uporabo 
    pts = np.ones((3, U.size))
    pts[0], pts[1] = U, V

    # transformiranje kontrolnih točk z izračunano matriko
    oCP = np.linalg.inv(oT) @ pts

    # vzamemo relevantne kooeficiente iz matrike a11, a12, a21, a22
    oCP = oCP[:2, :].T

    # transformiramo vhodno sliko z izračunano matriko
...
def getImages(iImage1, iImage2, iStep):
    ...
    # izračunamo šahovnico
    Y, X = iImage2.shape
    oImage2 = iImage1.copy()

    for y in range(0, Y, iStep):
        for x in range(0, X, iStep):
            isDiag = (x + y) / iStep % 2 # % = ostanek pri deljenju
